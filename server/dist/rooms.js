...
import { resolveAndGoToDay, finishDayAndGoToNight, getNextRoleTurn, } from "./game/stateMachine.js";
...
export function startGame(roomCode) {
    ...
    const roleDeck = state.roleDeck.length === state.players.length
        ? state.roleDeck
        : [];
...
export function applyNightAction(roomCode, socketId, payload) {
    const state = rooms.get(roomCode);
    if (!state)
        return { success: false, error: "Room not found" };
...
export function applyHunterAction(roomCode, socketId, targetId) {
    const state = rooms.get(roomCode);
    if (!state)
        return { success: false, error: "Room not found" };
    ...
    if (!state.hunterPending)
        return { success: false, error: "No hunter pending" };
    const hunter = state.players.find((p) => p.id === state.hunterPending);
    if (!hunter || hunter.socketId !== socketId)
        return { success: false, error: "Not the hunter" };
...
export function advanceToDay(roomCode) {
    const state = rooms.get(roomCode);
...
export function submitCaptainVote(roomCode, socketId, targetId) {
    const state = rooms.get(roomCode);
    if (!state)
        return { success: false, error: "Room not found" };
    if (state.phase !== "captain")
        return { success: false, error: "Not captain phase" };
...
export function submitVote(roomCode, socketId, targetId) {
    const state = rooms.get(roomCode);
    if (!state)
        return { success: false, error: "Room not found" };
    if (state.phase !== "day")
...
export function executeLynch(roomCode) {
    const state = rooms.get(roomCode);
...
export function allHaveVoted(state) {
