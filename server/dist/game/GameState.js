...
const PRESET_CLASSIC_8 = ["wolf", "villager", "villager", "villager", "villager", "seer", "witch", "hunter"];
const PRESET_BIG_12 = [
    "wolf", "wolf", "villager", "villager", "villager", "villager", "villager",
    "seer", "witch", "hunter", "cupid", "protector",
];
export function createInitialGameState(roomCode, players, roleDeck = []) {
    ...
    let thiefChoices;
    let currentRoleTurn = "wolf";
    if (hasThief) {
        const pool = ["wolf", "seer", "witch", "hunter", "villager"];
...
export function getWolves(state) {
    return state.players.filter((p) => p.alive && p.role === "wolf");
}
export function getVillagers(state) {
    return state.players.filter((p) => p.alive && p.role !== "wolf");
}
...
export function checkWin(state) {
    ...
    if (wolves.length >= villagers.length)
        return "wolves";
    const lovers = alive.filter((p) => p.loverId && alive.some((a) => a.id === p.loverId));
    if (alive.length === 2 && lovers.length === 2) {
        const [a, b] = lovers;
        if ((a.role === "wolf") !== (b.role === "wolf"))
            return "lovers";
    }
    return null;
}
