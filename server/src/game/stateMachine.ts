...
import { getAlivePlayers, getWolves, checkWin } from "./GameState.js";
import { resolveNight } from "./resolution.js";

const NIGHT_ORDER: Role[] = ["wolf", "seer", "witch", "protector"];
const NIGHT_1_PREFIX: Role[] = ["thief", "cupid"];

export function getNextRoleTurn(state: GameState): Role | null {
...
export function getFirstNightRole(state: GameState): Role | null {
  const prefix = state.nightNumber === 1 ? NIGHT_1_PREFIX : [];
  const order = [...prefix, ...NIGHT_ORDER];
...
export function advanceNightPhase(state: GameState): Partial<GameState> {
...
export function resolveAndGoToDay(state: GameState): GameState {
...
export function finishDayAndGoToNight(state: GameState): GameState {
